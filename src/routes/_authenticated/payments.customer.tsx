  ...
  const history = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("id, occurred_at, amount, note, voided_at, void_reason, customers(name)")
        ...
        <CardContent className="pt-6 overflow-auto">
          ...
          <Table>
            <TableHeader><TableRow>
              <TableHead>Waktu</TableHead><TableHead>Pelanggan</TableHead><TableHead className="text-right">Nominal</TableHead><TableHead>Catatan</TableHead><TableHead className="text-right">Aksi</TableHead>
              ...
              {(history.data ?? []).map((r: any, i) => (
                <TableRow key={i} className={r.voided_at ? "opacity-50" : ""}>
                  ...
                  <TableCell className="text-right">
                    <VoidButton table="customer_payments" id={r.id} voidedAt={r.voided_at} voidReason={r.void_reason} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
